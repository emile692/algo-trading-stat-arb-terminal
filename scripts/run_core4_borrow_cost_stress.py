...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Core 4 borrow-cost stress tests on reconstructed short exposure proxies.")
...
def main() -> int:
    ...
    bundle = run_core4_borrow_cost_stress(options, project_root=PROJECT_ROOT, output_dir=Path(output_dir))
    logging.info("Core 4 borrow-cost stress written to %s", output_dir)
    logging.info("Scenario count: %s", len(bundle["borrow_cost_stress"]))
    return 0
